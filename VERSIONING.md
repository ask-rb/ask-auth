...
Patch numbers advance by exactly one per release: `0.3.4` → `0.3.5` →
`0.3.6`. Never skip or jump patch numbers (`0.3.4` → `0.3.7` is wrong), even
...
gemchain guard ask-auth
gemchain update ask-auth <new-version> --dry-run
gemchain update ask-auth <new-version>
```
...
3. Build passes — `bundle exec rake build` (or `gem build ask-auth.gemspec`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/auth/version.rb` by gemchain, not by hand.
...
| `lib/ask/auth/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
- Bug fix only: `0.3.4` → `0.3.5`.
- Several bug fixes ship together: one release, `0.3.4` → `0.3.5` (never
  `0.3.7`).
- New backwards-compatible API: `0.3.5` → `0.4.0`.
...
- This gem bumped because a dependency released: patch only, e.g. `0.3.5` →
  `0.3.6`, via the gemchain cascade — never hand-published.
